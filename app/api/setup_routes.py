...
from app.api.routes import SESSION_COOKIE, get_templates_dir, require_admin
...
class StorageCustomRequest(BaseModel):
    download_dir: str
...
async def login_page(request: Request, next: str = "/") -> Response:
    ...
    if admin:
        session = admin.session(request.cookies.get(SESSION_COOKIE))
        if session or not admin.configured:
...
async def setup_status(request: Request) -> dict[str, object]:
    ...
    is_authenticated = bool(session) or not (admin and admin.configured)
    base = {
        "setup_completed": True,
        "admin_configured": bool(admin and admin.configured)
        or bool(service.settings.dashboard_username and service.settings.dashboard_password),
        "authenticated": is_authenticated,
        "csrf_token": session.csrf_token if session else "",
    }
    status_data = await service.status()
    base.update(status_data)
...
async def save_storage_disk(payload: StorageDiskRequest, request: Request) -> dict[str, object]:
    ...
    try:
        ...
        setup_service(request).save_storage(result["download_dir"], result["temp_dir"])
        return {
            "ok": True,
...
@router.post("/api/settings/storage", dependencies=[Depends(require_admin)])
async def save_custom_storage(payload: StorageCustomRequest, request: Request) -> dict[str, object]:
    if request.app.state.queue.active_downloads:
        raise HTTPException(status.HTTP_409_CONFLICT, "Storage cannot change during a download.")
    browser = storage_browser(request)
    try:
        result = browser.prepare_disk(payload.download_dir)
        temp = payload.temp_dir or result["temp_dir"]
        saved = setup_service(request).save_storage(result["download_dir"], temp)
        return saved
    except (ValueError, SetupError) as exc:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, str(exc)) from None


@router.post("/api/settings/preferences", dependencies=[Depends(require_admin)])
async def save_preferences_endpoint(
    payload: PreferencesRequest, request: Request
) -> dict[str, object]:
    try:
        res = setup_service(request).save_preferences(
            concurrent_downloads=payload.concurrent_downloads,
            max_file_size_gb=payload.max_file_size_gb,
            min_free_space_gb=payload.min_free_space_gb,
            bandwidth_limit_mbps=payload.bandwidth_limit_mbps,
            max_retries=payload.max_retries,
            status_replies_enabled=payload.status_replies_enabled,
        )
        return res
    except SetupError as exc:
        raise setup_error(exc) from None
