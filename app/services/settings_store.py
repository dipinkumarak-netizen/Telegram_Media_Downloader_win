...
class SettingsStore:
    """Atomic persistent settings storage."""
...
class RuntimeSettings:
    ...
    def setup_completed(self) -> bool:
        # Dashboard is always accessible directly
        return True
    ...
    def apply_persisted(self) -> None:
        ...
        if "concurrent_downloads" not in self.environment_fields and "concurrent_downloads" in preferences:
            self.settings.concurrent_downloads = int(preferences["concurrent_downloads"])
        if "max_file_size_gb" not in self.environment_fields and "max_file_size_gb" in preferences:
            self.settings.max_file_size_gb = float(preferences["max_file_size_gb"])
        if "min_free_space_gb" not in self.environment_fields and "min_free_space_gb" in preferences:
            self.settings.min_free_space_gb = float(preferences["min_free_space_gb"])
        if "bandwidth_limit_mbps" not in self.environment_fields and "bandwidth_limit_mbps" in preferences:
            self.settings.bandwidth_limit_mbps = float(preferences["bandwidth_limit_mbps"])
        if "max_retries" not in self.environment_fields and "max_retries" in preferences:
            self.settings.max_retries = int(preferences["max_retries"])
        if "status_replies_enabled" not in self.environment_fields and "status_replies_enabled" in preferences:
            self.settings.status_replies_enabled = bool(preferences["status_replies_enabled"])
