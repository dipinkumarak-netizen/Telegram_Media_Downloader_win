...
def default_data_dir() -> Path:
    if os.name == "nt":
        ...
        if local_app_data:
            return Path(local_app_data) / "TelegramMediaDownloader" / "data"
        return Path.home() / ".telegram-media-downloader" / "data"
    return Path("/data")


def default_download_dir() -> Path:
    if os.name == "nt":
        return Path.home() / "Downloads" / "TelegramDownloads"
...
class Settings(BaseSettings):
    ...
    ``TMD_DATA_DIR`` and ``TMD_DOWNLOAD_DIR``, then fall back to platform-safe defaults.
    """
    ...
    data_dir: Path = Field(default_factory=default_data_dir, validation_alias="TMD_DATA_DIR")
    ...
    download_workers: int = Field(default=4, ge=1, le=8, validation_alias="TMD_DOWNLOAD_WORKERS")
    max_file_size_gb: float = Field(default=0, ge=0)
    min_free_space_gb: float = Field(default=2, ge=0)
