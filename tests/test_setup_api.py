...
async def test_fresh_install_loads_dashboard_directly_and_settings_accessible(tmp_path: Path) -> None:
    ...
    assert status_response.json()["setup_completed"] is True
    assert "Telegram Media Downloader" in root.text
...
async def test_completed_setup_uses_dedicated_login_and_existing_login_succeeds(
    ...
    assert before_login.json()["admin_configured"] is True
...
async def test_disk_selection_api_returns_roots_and_persists(
    tmp_path: Path,
) -> None:
    app = make_app(tmp_path)
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        selected = await client.post(
            "/api/setup/storage-disk", json={"mount_path": str(tmp_path / "downloads")}, headers=headers
        )
        reloaded = await client.get("/api/setup/status")

    assert len(disks.json()["disks"]) > 0
    assert selected.status_code == 200
    assert reloaded.json()["storage_root"] == str(tmp_path / "downloads")
...
async def test_setup_completion_locks_admin_creation(tmp_path: Path) -> None:
    ...
    assert second_admin.status_code in {400, 403}
